...
def calc_iou(a, b):
    ...
    iw = torch.min(torch.unsqueeze(a[:, 2], dim=1), b[:, 2]) - torch.max(
        torch.unsqueeze(a[:, 0], 1), b[:, 0]) + 1
    ih = torch.min(torch.unsqueeze(a[:, 3], dim=1), b[:, 3]) - torch.max(
        torch.unsqueeze(a[:, 1], 1), b[:, 1]) + 1
...
class APLoss(torch.autograd.Function):
    ...
    def forward(ctx, classifications, regressions, anchors, gt_bboxes,
        ...
        for j in range(batch_size):
            ...
            bbox_annotation = torch.cat(
                [gt_bboxes[j], gt_labels[j][:, None].float()], -1)
            ...
            positive_indices[IoU == gt_IoU_max] = True
            ######
            ...
            if positive_indices.sum() > 0:
                ...
                gt_ctr_y = assigned_annotations[:, 1] + 0.5 * (
                    gt_heights - 1.0)
                ...
                regression_grad = torch.where(
                    torch.le(regression_diff_abs, 1.0 / 1.0),
                    1.0 * regression_diff, torch.sign(regression_diff))
        ...
        classification_grads, classification_losses = AP_loss(
            classifications, labels_b)
        #########################################################

        ctx.save_for_backward(classification_grads, regression_grads)
        return classification_losses, torch.stack(regression_losses).sum(
            dim=0, keepdim=True) / p_num
